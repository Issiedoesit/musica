...
const navToggle = () =>{
    ...
    if (navRef.current.classList.contains('-translate-x-[150%]')){
    ...
    return(
        <header className='sticky md:static top-0 left-0 z-[200]'>
            <nav id='nav' ref={navRef}  className='-translate-x-[150%] landscape:overflow-y-auto shadow-sm shadow-slate-500 md:bg-transparent md:shadow-none  md:translate-x-0 transition-transform duration-1000 flex z-50 flex-col md:gap-5 bg-dark-blue h-screen fixed top-0 left-0 w-9/12 md:w-24 md:items-center pb-32 pl-3 md:pl-0 pt-[20%] md:pt-[72px]'>
            ...
            <div className='space-y-10 px-[15px] py-5 rounded-[32px] md:bg-dark-blue-alt flex flex-col'>
                <NavLink  exact to='/' end={true} id='home'  className={({ isActive }) => (isActive ? 'flex flex-row items-center gap-10 current-nav' : 'not-current-nav flex flex-row items-center gap-10 ')}>
            ...
            <motion.div id='search_menu' className='w-full flex flex-row relative z-[250] items-center justify-between gap-10 backdrop-blur-2xl md:backdrop-blur-none px-6 md:pr-6 md:pl-24'>
                <div className="flex md:hidden flex-row items-center gap-10">
                    <svg id="menu" onClick={navToggle} className='group cursor-pointer' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
